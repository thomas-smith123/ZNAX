...
# Patch Embedding
def main(fold_idx=0, seed=42):
    set_seed(seed)
    print(f"Running fold {fold_idx + 1}/5 with seed {seed}")
    
    # Initialize TensorBoard writer
    log_dir = f'./model_ZNAX/runs/fold_{fold_idx}_seed_{seed}'
    ...
    # 加载数据集（只加载一次）
    dataset_path = './dataset/dataset_training_aug.h5'
    ...
    labels = labels.flatten() - 1  # 确保从0开始索引
    ...
    finetune_train_dataset = SupervisedH5Dataset(supervised_train_data, supervised_train_labels, transform=finetune_transform)
    ...
    print(f"Fine-tuning - Training samples: {len(finetune_train_dataset)}, Test samples: {len(finetune_test_dataset)}")
    print(f"NOTE: Using independent supervised training data (no overlap with contrastive learning data)")
    ...
    # Attach Classifier
    num_classes = len(np.unique(supervised_train_labels))  # 使用监督学习训练标签计算类别数
    ...
    def train_finetune(model, train_loader, criterion, optimizer, scheduler, device, num_epochs, writer):
        for epoch in range(num_epochs):
            ...
            for batch_idx, (inputs, targets) in enumerate(train_loader):
                ...
                # Record batch-level metrics for supervised fine-tuning
                global_step_finetune = epoch * len(train_loader) + batch_idx + 1
                batch_acc = (predicted == targets).float().mean().item() * 100.0
            ...
            # Record epoch-level metrics for supervised fine-tuning
            writer.add_scalar('Supervised_Finetune/Epoch_Loss', epoch_loss, epoch)
            writer.add_scalar('Supervised_Finetune/Epoch_Accuracy', epoch_accuracy * 100.0, epoch)
            writer.add_scalar('Supervised_Finetune/Learning_Rate', optimizer.param_groups[0]['lr'], epoch)

            # ensure scalars are flushed to disk for TensorBoard
            try:
                writer.flush()
            except Exception:
                pass
    ...
    # Call the fine-tuning function with the correct data loader
    train_finetune(model, finetune_train_loader, criterion, optimizer, scheduler, device, num_epochs, writer)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gpus', type=str, default="5", help='GPU devices to use')
    ...
    if args.fold_idx >= 0:
        ...
        accuracy = main(fold_idx=args.fold_idx, seed=args.seed)
        ...
        for fold in range(5):
            print(f"\n{'='*20} FOLD {fold+1}/5 {'='*20}")
            # try:
            accuracy = main(fold_idx=fold, seed=args.seed)
        ...
        if all_accuracies:
            ...
            import json
            with open(f'./model_ZNAX/save/5fold_results_seed{args.seed}.json', 'w') as f:
                json.dump(results, f, indent=4)
            print(f"Results saved to: ./model_ZNAX/save/5fold_results_seed{args.seed}.json")
        else:
